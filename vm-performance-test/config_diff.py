...
def check_trunk_vlan(cfg, vlan_name):
    ...
    minVlan = Vlan(minVlanId, "%s_min" % vlan_name)
    maxVlan = Vlan(maxVlanId, "%s_max" % vlan_name)
                ...
                if user_input.strip() in ["y", "yes"]:
                    if mode == "trunk":
                        if intf.get_vlan_mode() != "trunk": 
                            intf.set_trunk()
                        intf.add_trunk_vlan(vlan_id)
                    elif mode == "access":
                        intf.set_access_vlan(vlan_id)
                    check = True
